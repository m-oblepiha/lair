...
const attackCounterCaseReducer = (
  ...
  target.relations[actor.id] = clipRelation(
    (target.relations[actor.id] ?? 0) - 0.1 * action.payload.value
  );
};
...
const attackAvengeCaseReducer = (
  ...
  actActor.relations[actor.id] = clipRelation(
    (actActor.relations[actor.id] ?? 0) - 0.1 * action.payload.value
  );

  if (actTarget)
    actTarget.relations[actor.id] = clipRelation(
      (actTarget.relations[actor.id] ?? 0) + 0.5
    );
};
...
const attackJoinCaseReducer = (
  ...
  actTarget.relations[actor.id] = clipRelation(
    (actTarget.relations[actor.id] ?? 0) - 0.1 * action.payload.value
  );

  actActor.relations[actor.id] = clipRelation(
    (actActor.relations[actor.id] ?? 0) + 0.2
  );
};
...
const bullyCounterCaseReducer = (
  ...
  target.relations[actor.id] = clipRelation(
    (target.relations[actor.id] ?? 0) - 0.02 * action.payload.value
  );
};
...
const bullyAvengeCaseReducer = (
  ...
  actActor.relations[actor.id] = clipRelation(
    (actActor.relations[actor.id] ?? 0) - 0.02 * action.payload.value
  );

  actTarget.relations[actor.id] = clipRelation(
    (actTarget.relations[actor.id] ?? 0) + 0.2
  );
};
...
const bullyJoinCaseReducer = (
  ...
  actTarget.relations[actor.id] = clipRelation(
    (actTarget.relations[actor.id] ?? 0) - 0.02 * action.payload.value
  );

  actActor.relations[actor.id] = clipRelation(
    (actActor.relations[actor.id] ?? 0) + 0.1
  );
};
